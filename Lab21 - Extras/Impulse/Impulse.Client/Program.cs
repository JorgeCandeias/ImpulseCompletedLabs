...
void PrintUsage()
{
    ...
    var markup = new Markup(
       ...
       + "[bold fuchsia]/s[/] to query [underline green]statistics[/] from the cluster\n"
...
async Task JoinChannel(string channelName)
{
    ...
    await AnsiConsole.Status().StartAsync("Joining channel...", async ctx =>
    {
        ...
        // get the stream corresponding to the channel
        var stream = client
            .GetStreamProvider("Chat")
            .GetStream<ChatMessage>(Guid.Empty, channelName);
...
{
    var stats = await client.GetChannelGlobalStatsReplicaGrain().GetStatsAsync();

    AnsiConsole.MarkupLine(
        "[bold yellow]Cluster Stats[/]: [bold olive]Channels[/]: {0}, [bold olive]Members[/]: {1}, [bold olive]Messages[/]: {2}",
        stats.Channels, stats.Members, stats.Messages);
}
...
async Task LeaveChannel()
{
    ...
    await AnsiConsole.Status().StartAsync("Leaving channel...", async ctx =>
    {
        ...
        var stream = client
            .GetStreamProvider("Chat")
            .GetStream<ChatMessage>(Guid.Empty, currentChannel);

        // unsubscribe all handles from the stream
...
void SetActivityId()
{
    ...
    AnsiConsole.MarkupLine($"[bold yellow]ActivityId:[/] {RequestContext.ActivityId}, [bold yellow]ClientId:[/] {clientId}");
}